...
class ForumManager
{
    ...
    public function registerRoles()
    {
     ...
     public function getFrontContent($locale, $contentType, $contentId, $result = null)
     {
         if ($contentType == 'view')
         {
             ...
             $params['editStatus'] = '';
             $params['editMessageValue'] = $params['content'];
             ...
             {
                 $message['editStatus'] = '';
                 $message['editMessageValue'] = $message['content'];
             ...
             if (isset($forum[0])) 
             {
                 ...
                 } else
                 {
                     ...
                     if ($errors == false)
                     {
                         ...
                         $result['status'] = 'OK';
                         // Запись в лог
                         $this->container->get('cms.cmsManager')->logPushFront(0, 'create', 'Отправка сообщения к форуму "'.$forum[0]['title'].'"', $this->container->get('router')->generate('forum_forum_messages_edit').'?id='.$messageent->getId());
             ...
             if ((!empty($forument)) && (($messageid == 0) || (!empty($messageent))))
             {
                 ...
                 } else
                 {
                     ...
                     if ($errors == false)
                     {
                         ...
                         if ($messageid == 0)
                         {
                            $this->container->get('cms.cmsManager')->logPushFront(0, 'edit', 'Изменение сообщения к форуму "'.$forument->getTitle().'"', $this->container->get('router')->generate('forum_forum_topic_edit').'?id='.$forument->getId());
                         } else
                         {
                            $this->container->get('cms.cmsManager')->logPushFront(0, 'edit', 'Изменение сообщения к форуму "'.$forument->getTitle().'"', $this->container->get('router')->generate('forum_forum_messages_edit').'?id='.$messageid);
                         }
                     } else $result['status'] = 'Check error';
                 }
             }
             ...
             if ((!empty($forument)) && (($messageid == 0) || (!empty($messageent))))
             {
                 ...
                 } else
                 {
                     ...
                     if ($errors == false)
                     {
                         ...
                         if ($messageid == 0)
                         {
                            $this->container->get('cms.cmsManager')->logPushFront(0, 'moderate', 'Модерация сообщения к форуму "'.$forument->getTitle().'"', $this->container->get('router')->generate('forum_forum_topic_edit').'?id='.$forument->getId());
                         } else
                         {
                            $this->container->get('cms.cmsManager')->logPushFront(0, 'moderate', 'Модерация сообщения к форуму "'.$forument->getTitle().'"', $this->container->get('router')->generate('forum_forum_messages_edit').'?id='.$messageid);
                         }
                         $result['status'] = 'OK';
                     } else $result['status'] = 'Check error';
                 }
         ...
         if (($action == 'closetopic') || ($action == 'importanttopic'))
         {
             ...
             if (!empty($forument))
             {
                 ...
                 } else
                 {
                     ...
                     $result['status'] = 'OK';
                     // Запись в лог
                     $this->container->get('cms.cmsManager')->logPushFront(0, 'edit', 'Изменение статуса форума "'.$forument->getTitle().'"', $this->container->get('router')->generate('forum_forum_topic_edit').'?id='.$forument->getId());
                 }
             }
         }
         if ($action == 'create')
             ...
             if ((!empty($createpage)) && ($createpage->getEnabled() != 0))
             {
                 ...
                 } else
                 {
                     ...
                     if ($errors == false)
                     {
                         ...
                         $forument->setCreaterId(($cmsManager->getCurrentUser() != null ? $cmsManager->getCurrentUser()->getId() : null));
                         ...
                         $result['status'] = 'OK';
                        // Запись в лог
                        $this->container->get('cms.cmsManager')->logPushFront(0, 'create', 'Создание форума "'.$forument->getTitle().'"', $this->container->get('router')->generate('forum_forum_topic_edit').'?id='.$forument->getId());
                     } else $result['status'] = 'Check error';
                 }
             }
         }
             ...
             if ($module != null) 
             {
                 ...
                 } else
                 {
                     ...
                     if ($errors == false)
                     {
                         ...
                         $result['status'] = 'OK';
                        // Запись в лог
                        $this->container->get('cms.cmsManager')->logPushFront(0, 'create', 'Создание комментария к странице "'.$seoPageEnt->getDescription().'"', $this->container->get('router')->generate('forum_forum_comments_edit').'?id='.$messageent->getId());
                     } else $result['status'] = 'Check error';
                 }
             }
         }
             ...
             if (($module != null) && (!empty($messageent)))
             {
                 ...
                 } else
                 {
                     ...
                     if ($errors == false)
                     {
                         ...
                         $result['status'] = 'OK';
                         // Запись в лог
                         $this->container->get('cms.cmsManager')->logPushFront(0, 'edit', 'Изменение комментария к странице "'.$seoPageEnt->getDescription().'"', $this->container->get('router')->generate('forum_forum_comments_edit').'?id='.$messageent->getId());
                     } else $result['status'] = 'Check error';
                 }
             }
         }
             ...
             if (($module != null) && (!empty($messageent)))
             {
                 ...
                 } else
                 {
                     ...
                     if ($errors == false)
                     {
                         ...
                         $result['status'] = 'OK';
                         // Запись в лог
                         $this->container->get('cms.cmsManager')->logPushFront(0, 'moderate', 'Модерация комментария к странице "'.$seoPageEnt->getDescription().'"', $this->container->get('router')->generate('forum_forum_comments_edit').'?id='.$messageent->getId());
                     } else $result['status'] = 'Check error';
                 }
             }
         }
     ...
     public function getFrontModule($locale, $seoPage, $moduleType, $parameters, $result = null)
     {
         ...
         if ($moduleType == 'comments')
         {
             ...
             $params['pageCount'] = 1;
             $params['preContent'] = (!empty($parameters['preContent']) ? $parameters['preContent'] : '');
             $params['postContent'] = (!empty($parameters['postContent']) ? $parameters['postContent'] : '');
    ...
    public function getAdminModuleController($request, $module, $actionType, $nullparameters = null)
    {
        if ($module == 'comments')
        {
            ...
            $parameterserror['preContent'] = '';
            $parameterserror['postContent'] = '';
            $errors = false;
            if ($request->getMethod() == 'POST')
            {
                ...
                if (isset($postparameters['preContent'])) $parameters['preContent'] = $postparameters['preContent'];
                if (isset($postparameters['postContent'])) $parameters['postContent'] = $postparameters['postContent'];
            ...
            if ($actionType == 'tab')
            {
                return $this->container->get('templating')->render('ForumForumBundle:Modules:comments.html.twig',array('parameters' => $parameters,'parameterserror' => $parameterserror,'moderators'=>$moderators));
            }
        }
        $answer = null;
        $cmsservices = $this->container->getServiceIds();
